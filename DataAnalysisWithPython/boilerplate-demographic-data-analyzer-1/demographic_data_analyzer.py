...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(higher_education['salary'].value_counts()[
        '>50K'] / higher_education.shape[0] * 100, 1)
    lower_education_rich = round(lower_education['salary'].value_counts()[
        '>50K'] / lower_education.shape[0] * 100, 1)
    ...
    num_min_workers = df[df['hours-per-week'] == min_hrs]

    rich_percentage = round(num_min_workers['salary'].value_counts()[
        '>50K'] / num_min_workers.shape[0] * 100, 1)
    ...
    for country, data in df.groupby('native-country'):
        earning_percent = (
            data['salary'] == '>50K').sum() / data['salary'].count()
        if highest_earning_country_percentage < earning_percent:
            highest_earning_country_percentage = earning_percent
